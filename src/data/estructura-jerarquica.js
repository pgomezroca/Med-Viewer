export const estructuraJerarquica = {
  "Cuello": {
    "Traumatica": {
      "Oseo": {
        "FX-CLAVICULA": [
          "Reducción y yeso",
          "Clavijas",
          "Tutores clásicos",
          "DET",
          "Placa-tornillo",
          "Alambre",
          "Otro"
        ]
      },
      "Vascular": {
        "Lesiones de la subclavia": [
          "Reparacion"
        ]
      },
      "Nervioso": {
        "Lesión de plexo braquial": [
          "Neurorrafia",
          "Neurolisis",
          "Injerto de nervio",
          "Otros"
        ]
      }
    },
    "Tumoral": {
      "Oseo": {
        "Tumor óseo": [
          "Resección",
          ""
        ]
      },
      "vascular": {
        "Tumor vascular": [
          "Reseccion"
        ]
      },
      "Nervioso": {
        "Tumor nervioso": [
          "Reseccion"
        ],
        "Metastasis": [
          "Reseccion"
        ]
      }
    }
  },
  "Hombro": {
    "Traumatica": {
      "Oseo": {
        "Fx de clavícula": [
          "Reducción y yeso",
          "Clavijas",
          "Tutores clásicos",
          "DET",
          "Placa-tornillo",
          "Alambre",
          "Otro"
        ],
        "Fx de escápula": [
          "Reducción y yeso",
          "Clavijas",
          "Tutores clásicos",
          "DET",
          "Placa-tornillo",
          "Alambre",
          "Otro"
        ],
        "Fx húmero-proximal": [
          "Reducción y yeso",
          "Clavijas",
          "Tutores clásicos",
          "DET",
          "Placa-tornillo",
          "Alambre",
          "Otro"
        ]
      },
      "Vascular": {
        "Lesión arteria subclavia": [
          "Sutura",
          "Ligadura",
          "Interposición"
        ],
        "Lesión arteria humeral": [
          "Sutura",
          "Ligadura",
          "Interposición"
        ],
        "Otras": [
          "Sutura",
          "Ligadura",
          "Interposición"
        ]
      },
      "Nervioso": {
        "Plexo": [
          "Neurorrafia",
          "Neurolisis",
          "Injerto de nervio",
          "Otros"
        ],
        "Nervio circunflejo": [
          "Neurorrafia",
          "Neurolisis",
          "Injerto de nervio",
          "Otros"
        ],
        "Nervio radial": [
          "Neurorrafia",
          "Neurolisis",
          "Injerto de nervio",
          "Otros"
        ],
        "Nervio músculo-cutáneo": [
          "Neurorrafia",
          "Neurolisis",
          "Injerto de nervio",
          "Otros"
        ],
        "Nervio mediano": [
          "Neurorrafia",
          "Neurolisis",
          "Injerto de nervio",
          "Otros"
        ],
        "Nervio cubital": [
          "Neurorrafia",
          "Neurolisis",
          "Injerto de nervio",
          "Otros"
        ]
      },
      "Musculo-tendinoso": {
        "Manguito rotador": [
          "Transferencia",
          "injerto",
          "sutura"
        ],
        "Biceps proximal": [
          "Transferencia",
          "Injerto",
          "Sutura"
        ],
        "Subescapular": [
          "Transferencia",
          "Injerto",
          "Sutura"
        ],
        "Pectoral mayor": [
          "Transferencia",
          "Injerto",
          "Sutura"
        ]
      },
      "Articular": {
        "Acromio-clavicular": [
          "Artroplastia",
          "Reemplazo parcial",
          "Reemplazo total",
          "Artroscopía"
        ],
        "Gleno-humeral": [
          "Artroplastia",
          "Reemplazo parcial",
          "Reemplazo total",
          "Artroscopía"
        ],
        "Escapular": [
          "Artroplastia",
          "Reemplazo parcial",
          "Reemplazo total",
          "Artroscopía"
        ]
      }
    },
    "Tumoral": {
      "Oseo": {
        "Tumor primario": [
          "Reseccion",
          "protesis"
        ],
        "Tumor secundario": [
          "Reseccion",
          "Protesis"
        ]
      },
      "Vascular": {
        "Tumor primario": [
          "Reseccion",
          "Protesis"
        ],
        "Tumor secundario": [
          "Reseccion",
          "Protesis"
        ]
      },
      "Nervioso": {
        "Tumor primario": [
          "Reseccion",
          "Protesis"
        ],
        "Tumor secundario": [
          "Reseccion",
          "Protesis"
        ]
      },
      "Musculo-tendinoso": {
        "Tumor primario": [
          "Reseccion",
          "protesis"
        ],
        "tumor secundario": [
          "Reseccion",
          "Protesis"
        ]
      },
      "Articular": {
        "Tumor primario": [
          "Reseccion",
          "Protesis"
        ],
        "Tumor secundario": [
          "Reseccion",
          "Protesis"
        ]
      }
    },
    "Degenerativo": {
      "Articular": {
        "Artritis": [
          "Reconstrucción",
          "Reemplazo protésico",
          "Artrodesis"
        ],
        "Artrosis": [
          "Reconstrucción",
          "Reemplazo protésico",
          "Artrodesis"
        ],
        "Necrosis": [
          "Reconstrucción",
          "Reemplazo protésico",
          "Artrodesis"
        ]
      }
    },
    "Infeccioso": {
      "Todos": {
        "Artritis séptica": [
          "Toilette",
          "Resección",
          "Colgajo"
        ],
        "ARTRITIS ASEPTICA": [
          "Toilette",
          "Resección",
          "Colgajo"
        ]
      }
    }
  },
  "Brazo": {
    "Traumatica": {
      "Oseo": {
        "Fx diafisaria de húmero": [
          "Reducción y yeso",
          "Clavijas",
          "Tutores clásicos",
          "DET",
          "Placa-tornillo",
          "Alambre",
          "Otro"
        ],
        "Fx-subtrocantérica distal del húmero": [
          "Reducción y yeso",
          "Clavijas",
          "Tutores clásicos",
          "DET",
          "Placa-tornillo",
          "Alambre",
          "Otro"
        ],
        "Fx-patológica en brazo": [
          "Reducción y yeso",
          "Clavijas",
          "Tutores clásicos",
          "DET",
          "Placa-tornillo",
          "Alambre",
          "Otro"
        ]
      },
      "Vascular": {
        "Lesión arteria braquial": [
          "Sutura",
          "Ligadura",
          "Interposición"
        ],
        "Lesión arteria colateral radial o cubital": [
          "Sutura",
          "Ligadura",
          "Interposición"
        ]
      },
      "Nervioso": {
        "Nervio radial (fractura húmero)": [
          "Neurorrafia",
          "Neurolisis",
          "Injerto de nervio",
          "Otros"
        ],
        "Nervio mediano": [
          "Neurorrafia",
          "Neurolisis",
          "Injerto de nervio",
          "Otros"
        ],
        "Nervio cubital": [
          "Neurorrafia",
          "Neurolisis",
          "Injerto de nervio",
          "Otros"
        ]
      },
      "Musculo-tendinoso": {
        "Bíceps braquial": [
          "Reinserción distal",
          "Injerto tendinoso",
          "Transferencia"
        ],
        "Tríceps braquial": [
          "Reinserción",
          "Injerto tendinoso",
          "Transferencia"
        ],
        "Braquial anterior": [
          "Sutura",
          "Injerto",
          "Transferencia"
        ]
      },
      "Articular": {
        "Articulación húmero-humeral": [
          "Reconstrucción",
          "Artrodesis",
          "Artroscopía"
        ]
      }
    },
    "Tumoral": {
      "Oseo": {
        "Tumor primario": [
          "Resección",
          "Prótesis",
          "Injerto"
        ],
        "Tumor secundario": [
          "Resección",
          "Prótesis",
          "Injerto"
        ]
      },
      "Vascular": {
        "Tumor primario": [
          "Resección",
          "Ligadura"
        ],
        "Tumor secundario": [
          "Resección",
          "Ligadura"
        ]
      },
      "Nervioso": {
        "Tumor primario": [
          "Resección",
          "Neurólisis",
          "Injerto"
        ],
        "Tumor secundario": [
          "Resección",
          "Neurólisis",
          "Injerto"
        ]
      },
      "Musculo-tendinoso": {
        "Tumor primario": [
          "Resección",
          "Reparación tendinosa"
        ],
        "Tumor secundario": [
          "Resección",
          "Reparación tendinosa"
        ]
      },
      "Articular": {
        "Tumor primario": [
          "Resección",
          "Artrodesis"
        ],
        "Tumor secundario": [
          "Resección",
          "Artrodesis"
        ]
      }
    },
    "Degenerativo": {
      "Articular": {
        "Pseudoartrosis diafisaria": [
          "Reconstrucción",
          "Injerto óseo",
          "Fijación interna"
        ],
        "Artrosis humeral por secuelas traumáticas": [
          "Reconstrucción",
          "Prótesis parcial",
          "Artrodesis"
        ]
      }
    },
    "Infeccioso": {
      "Todos": {
        "Osteomielitis diafisaria de húmero": [
          "Debridamiento",
          "Resección ósea",
          "Colgajo"
        ],
        "Artritis séptica (epífisis distal o proximal)": [
          "Toilette",
          "Resección",
          "Colgajo"
        ]
      }
    }
  },
  
  "Codo": {
    "Traumatica": {
      "Oseo": {
        "Fx epitróclea": [
          "Reducción y yeso",
          "Clavijas",
          "Placa-tornillo",
          "DET",
          "Alambre",
          "Otro"
        ],
        "Fx-epicóndilo": [
          "Reducción y yeso",
          "Clavijas",
          "Placa-tornillo",
          "DET",
          "Alambre",
          "Otro"
        ],
        "Fx-olecranon": [
          "Reducción y yeso",
          "Tornillos canulados",
          "Placa-tornillo",
          "DET",
          "Alambre en 8",
          "Otro"
        ],
        "Fx de cabeza radial": [
          "Reducción y yeso",
          "Resección",
          "Osteosíntesis",
          "Prótesis radial",
          "Otro"
        ],
        "Fx-luxación de Monteggia": [
          "Reducción y yeso",
          "Osteosíntesis cúbito",
          "Reconstrucción cabeza radial",
          "Otro"
        ]
      },
      "Vascular": {
        "Lesión arteria braquial": [
          "Sutura",
          "Ligadura",
          "Interposición"
        ],
        "Lesión arteria radial": [
          "Sutura",
          "Ligadura",
          "Interposición"
        ],
        "Lesión arteria cubital": [
          "Sutura",
          "Ligadura",
          "Interposición"
        ]
      },
      "Nervioso": {
        "Nervio radial": [
          "Neurorrafia",
          "Neurolisis",
          "Injerto de nervio",
          "Otros"
        ],
        "Nervio cubital": [
          "Neurorrafia",
          "Neurolisis",
          "Injerto de nervio",
          "Otros"
        ],
        "Nervio mediano": [
          "Neurorrafia",
          "Neurolisis",
          "Injerto de nervio",
          "Otros"
        ]
      },
      "Musculo-tendinoso": {
        "Tendón distal del bíceps": [
          "Reinserción directa",
          "Injerto tendinoso",
          "Otro"
        ],
        "Tríceps distal": [
          "Reinserción directa",
          "Injerto tendinoso",
          "Otro"
        ]
      },
      "Articular": {
        "Articulación húmero-radial": [
          "Artroplastia",
          "Reconstrucción",
          "Artroscopía"
        ],
        "Articulación húmero-cubital": [
          "Artroplastia",
          "Reconstrucción",
          "Artroscopía"
        ],
        "Articulación radiocubital proximal": [
          "Artroplastia",
          "Reconstrucción",
          "Artroscopía"
        ]
      }
    },
    "Tumoral": {
      "Oseo": {
        "Tumor primario": [
          "Resección",
          "Prótesis"
        ],
        "Tumor secundario": [
          "Resección",
          "Prótesis"
        ]
      },
      "vascular": {
        "Tumor primario": [
          "Resección",
          "Prótesis"
        ],
        "Tumor secundario": [
          "Resección",
          "Prótesis"
        ]
      },
      "Nervioso": {
        "Tumor primario": [
          "Resección",
          "Prótesis"
        ],
        "Tumor secundario": [
          "Resección",
          "Prótesis"
        ]
      },
      "Musculo-tendinoso": {
        "Tumor primario": [
          "Resección",
          "Prótesis"
        ],
        "Tumor secundario": [
          "Resección",
          "Prótesis"
        ]
      },
      "articular": {
        "Tumor primario": [
          "Resección",
          "Prótesis"
        ],
        "Tumor secundario": [
          "Resección",
          "Prótesis"
        ]
      }
    },
    "degenerativo": {
      "articular": {
        "Artritis reumatoidea": [
          "Reconstrucción",
          "Reemplazo protésico total",
          "Artrodesis"
        ],
        "Artrosis postraumática": [
          "Reconstrucción",
          "Reemplazo protésico parcial",
          "Artrodesis"
        ],
        "Osteonecrosis cabeza radial": [
          "Resección",
          "Prótesis radial",
          "Artrodesis"
        ]
      }
    },
    "infeccioso": {
      "todos": {
        "Artritis séptica": [
          "Toilette articular",
          "Resección",
          "Colgajo"
        ],
        "Osteomielitis proximal de cúbito o radio": [
          "Debridamiento",
          "Resección ósea",
          "Colgajo"
        ]
      }
    }
  },
  
  "Antebrazo": {
    "traumatica": {
      "oseo": {
        "Fx de radio": [
          "Reducción y yeso",
          "Clavijas",
          "Placa-tornillo",
          "DET",
          "Otro"
        ],
        "Fx de cúbito": [
          "Reducción y yeso",
          "Clavijas",
          "Placa-tornillo",
          "DET",
          "Otro"
        ],
        "Fx radio-cúbito ambos huesos": [
          "Reducción y yeso",
          "Osteosíntesis doble",
          "DET bilateral",
          "Otro"
        ],
        "Fx-luxación de Galeazzi": [
          "Osteosíntesis radio",
          "Reducción DRUJ",
          "Inmovilización",
          "Otro"
        ],
        "Fx-luxación de Monteggia": [
          "Osteosíntesis cúbito",
          "Reducción cabeza radial",
          "DET",
          "Otro"
        ]
      },
      "vascular": {
        "Lesión arteria radial": [
          "Sutura",
          "Ligadura",
          "Interposición"
        ],
        "Lesión arteria cubital": [
          "Sutura",
          "Ligadura",
          "Interposición"
        ]
      },
      "nervioso": {
        "Nervio mediano": [
          "Neurorrafia",
          "Neurolisis",
          "Injerto de nervio",
          "Otros"
        ],
        "Nervio radial": [
          "Neurorrafia",
          "Neurolisis",
          "Injerto de nervio",
          "Otros"
        ],
        "Nervio cubital": [
          "Neurorrafia",
          "Neurolisis",
          "Injerto de nervio",
          "Otros"
        ]
      },
      "musculo-tendinoso": {
        "Pronador redondo": [
          "Sutura",
          "Injerto",
          "Transferencia"
        ],
        "Flexores superficiales": [
          "Sutura",
          "Injerto",
          "Transferencia"
        ],
        "Extensores del antebrazo": [
          "Sutura",
          "Injerto",
          "Transferencia"
        ]
      },
      "articular": {
        "Articulación radiocubital proximal": [
          "Reconstrucción",
          "Artroplastia",
          "Artrodesis"
        ],
        "Articulación radiocubital distal": [
          "Reconstrucción",
          "Artroplastia",
          "Artrodesis"
        ]
      }
    },
    "tumoral": {
      "oseo": {
        "Tumor primario": [
          "Resección",
          "Prótesis",
          "Injerto"
        ],
        "Tumor secundario": [
          "Resección",
          "Prótesis",
          "Injerto"
        ]
      },
      "vascular": {
        "Tumor primario": [
          "Resección",
          "Prótesis"
        ],
        "Tumor secundario": [
          "Resección",
          "Prótesis"
        ]
      },
      "nervioso": {
        "Tumor primario": [
          "Resección",
          "Prótesis"
        ],
        "Tumor secundario": [
          "Resección",
          "Prótesis"
        ]
      },
      "musculo-tendinoso": {
        "Tumor primario": [
          "Resección",
          "Prótesis"
        ],
        "Tumor secundario": [
          "Resección",
          "Prótesis"
        ]
      },
      "articular": {
        "Tumor primario": [
          "Resección",
          "Prótesis"
        ],
        "Tumor secundario": [
          "Resección",
          "Prótesis"
        ]
      }
    },
    "degenerativo": {
      "articular": {
        "Inestabilidad radiocubital distal": [
          "Reconstrucción",
          "Artrodesis",
          "Prótesis DRUJ"
        ],
        "Artrosis radiocubital proximal": [
          "Reconstrucción",
          "Artrodesis",
          "Resección"
        ]
      }
    },
    "infeccioso": {
      "todos": {
        "Osteomielitis de cúbito o radio": [
          "Debridamiento",
          "Resección ósea",
          "Colgajo"
        ],
        "Artritis séptica radiocubital": [
          "Toilette",
          "Resección",
          "Colgajo"
        ]
      }
    }
  },
  "Muñeca": {
    "traumatica": {
      "oseo": {
        "Fx de escafoides": [
          "Reducción y yeso",
          "Clavijas",
          "Tutores clásicos",
          "DET",
          "Placa-tornillo",
          "Alambre",
          "Otro"
        ],
        "Fx de estiloides radial": [
          "Reducción y yeso",
          "Clavijas",
          "Placa-tornillo",
          "Otro"
        ],
        "Fx de estiloides cubital": [
          "Reducción y yeso",
          "Clavijas",
          "Placa-tornillo",
          "Otro"
        ],
        "Fx-luxación perilunar": [
          "Reducción cerrada",
          "Reducción abierta y fijación",
          "Artrodesis parcial",
          "Otro"
        ],
        "Fractura del semilunar": [
          "Reducción y yeso",
          "Osteosíntesis",
          "Otro"
        ]
      },
      "vascular": {
        "Lesión arteria radial": [
          "Sutura",
          "Ligadura",
          "Interposición"
        ],
        "Lesión arteria cubital": [
          "Sutura",
          "Ligadura",
          "Interposición"
        ]
      },
      "nervioso": {
        "Nervio mediano (síndrome compartimental o compresión aguda)": [
          "Neurólisis",
          "Descompresión",
          "Neurorrafia",
          "Injerto de nervio"
        ],
        "Nervio cubital": [
          "Neurorrafia",
          "Neurolisis",
          "Injerto de nervio"
        ]
      },
      "musculo-tendinoso": {
        "Flexores (FCR, FP, FDS)": [
          "Sutura",
          "Transferencia",
          "Injerto"
        ],
        "Extensores (ECRL, ECRB, ECU)": [
          "Sutura",
          "Transferencia",
          "Injerto"
        ]
      },
      "articular": {
        "Articulación radio-carpiana": [
          "Reconstrucción",
          "Artrodesis",
          "Artroscopía"
        ],
        "Articulación mediocarpiana": [
          "Reconstrucción",
          "Artrodesis",
          "Artroscopía"
        ]
      }
    },
    "tumoral": {
      "oseo": {
        "Tumor primario": [
          "Resección",
          "Artrodesis",
          "Injerto"
        ],
        "Tumor secundario": [
          "Resección",
          "Artrodesis",
          "Injerto"
        ]
      },
      "vascular": {
        "Tumor primario": [
          "Resección",
          "Ligadura"
        ],
        "Tumor secundario": [
          "Resección",
          "Ligadura"
        ]
      },
      "nervioso": {
        "Tumor primario": [
          "Resección",
          "Neurólisis",
          "Injerto"
        ],
        "Tumor secundario": [
          "Resección",
          "Neurólisis",
          "Injerto"
        ]
      },
      "musculo-tendinoso": {
        "Tumor primario": [
          "Resección",
          "Injerto tendinoso"
        ],
        "Tumor secundario": [
          "Resección",
          "Injerto tendinoso"
        ]
      },
      "articular": {
        "Tumor primario": [
          "Resección",
          "Artrodesis"
        ],
        "Tumor secundario": [
          "Resección",
          "Artrodesis"
        ]
      }
    },
    "degenerativo": {
      "articular": {
        "Artrosis radio-carpiana": [
          "Artroplastia",
          "Artrodesis",
          "Resección fila proximal"
        ],
        "Colapso del escafoides (SNAC)": [
          "Artrodesis parcial",
          "Artroplastia",
          "Resección fila proximal"
        ],
        "Colapso del semilunar (SLAC)": [
          "Artrodesis parcial",
          "Artroplastia",
          "Resección fila proximal"
        ]
      }
    },
    "infeccioso": {
      "todos": {
        "Artritis séptica carpiana": [
          "Toilette",
          "Resección carpiana",
          "Colgajo"
        ],
        "Osteomielitis carpal": [
          "Debridamiento",
          "Resección ósea",
          "Colgajo"
        ]
      }
    }
  },
  "Carpo": {
    "traumatica": {
      "oseo": {
        "Fx de escafoides": [
          "Reducción y yeso",
          "Clavijas",
          "Tutores clásicos",
          "DET",
          "Placa-tornillo",
          "Alambre",
          "Otro"
        ],
        "Fx de semilunar": [
          "Yeso",
          "Osteosíntesis",
          "Artrodesis",
          "Otro"
        ],
        "Fx de piramidal, pisiforme, trapecio, trapezoide, grande o ganchoso": [
          "Yeso",
          "Osteosíntesis",
          "Artrodesis",
          "Otro"
        ],
        "Fx múltiple carpiana": [
          "Reducción abierta",
          "Osteosíntesis múltiple",
          "Fijación externa",
          "Otro"
        ],
        "Luxación perilunar o lunopiramidal": [
          "Reducción cerrada",
          "Reducción abierta",
          "Fijación con clavijas",
          "Otro"
        ]
      },
      "vascular": {
        "Lesión arterial carpal (colateral radial o cubital profunda)": [
          "Sutura",
          "Ligadura",
          "Interposición"
        ]
      },
      "nervioso": {
        "Nervio mediano (síndrome compartimental o compresión aguda)": [
          "Descompresión",
          "Neurólisis",
          "Neurorrafia",
          "Injerto"
        ]
      },
      "musculo-tendinoso": {
        "Tendones extensores o flexores en región carpal": [
          "Sutura",
          "Transferencia",
          "Injerto"
        ]
      },
      "articular": {
        "Articulación intercarpiana": [
          "Artrodesis",
          "Artroscopía",
          "Reconstrucción ligamentaria"
        ],
        "Articulación radio-carpiana": [
          "Artrodesis parcial",
          "Reconstrucción ligamentaria",
          "Artroscopía"
        ]
      }
    },
    "tumoral": {
      "oseo": {
        "Tumor primario": [
          "Resección",
          "Injerto",
          "Artrodesis"
        ],
        "Tumor secundario": [
          "Resección",
          "Injerto",
          "Artrodesis"
        ]
      },
      "vascular": {
        "Tumor primario": [
          "Resección",
          "Ligadura"
        ],
        "Tumor secundario": [
          "Resección",
          "Ligadura"
        ]
      },
      "nervioso": {
        "Tumor primario": [
          "Resección",
          "Neurólisis",
          "Injerto"
        ],
        "Tumor secundario": [
          "Resección",
          "Neurólisis",
          "Injerto"
        ]
      },
      "musculo-tendinoso": {
        "Tumor primario": [
          "Resección",
          "Injerto tendinoso"
        ],
        "Tumor secundario": [
          "Resección",
          "Injerto tendinoso"
        ]
      },
      "articular": {
        "Tumor primario": [
          "Resección",
          "Artrodesis"
        ],
        "Tumor secundario": [
          "Resección",
          "Artrodesis"
        ]
      }
    },
    "degenerativo": {
      "articular": {
        "Colapso del escafoides (SNAC wrist)": [
          "Artrodesis parcial",
          "Resección fila proximal",
          "Artroplastia"
        ],
        "Colapso del semilunar (SLAC wrist)": [
          "Artrodesis parcial",
          "Resección fila proximal",
          "Artroplastia"
        ],
        "Artrosis intercarpiana": [
          "Artrodesis",
          "Artroplastia",
          "Toilette articular"
        ]
      }
    },
    "infeccioso": {
      "todos": {
        "Osteomielitis carpiana": [
          "Debridamiento",
          "Resección ósea",
          "Colgajo"
        ],
        "Artritis séptica intercarpiana": [
          "Toilette articular",
          "Resección",
          "Colgajo"
        ]
      }
    }
  },
  "Metacarpo": {
    "traumatica": {
      "oseo": {
        "Fx cuello de metacarpiano (Boxer)": [
          "Reducción y yeso",
          "Clavijas",
          "Tutores clásicos",
          "DET",
          "Placa-tornillo",
          "Alambre",
          "Otro"
        ],
        "Fx diáfisis de metacarpiano": [
          "Yeso",
          "Osteosíntesis con placa",
          "Clavijas",
          "DET",
          "Otro"
        ],
        "Fx base de metacarpiano": [
          "Yeso",
          "Clavijas",
          "Placa-tornillo",
          "Otro"
        ],
        "Fx múltiple metacarpiana": [
          "Yeso",
          "Fijación externa",
          "Osteosíntesis múltiple",
          "Otro"
        ]
      },
      "vascular": {
        "Lesión arterial dorsal o palmar metacarpiana": [
          "Sutura",
          "Ligadura",
          "Interposición"
        ]
      },
      "nervioso": {
        "Ramas digitales o metacarpianas": [
          "Neurorrafia",
          "Neurolisis",
          "Injerto de nervio"
        ]
      },
      "musculo-tendinoso": {
        "Tendones extensores metacarpianos": [
          "Sutura",
          "Transferencia",
          "Injerto"
        ],
        "Tendones flexores metacarpianos": [
          "Sutura",
          "Transferencia",
          "Injerto"
        ]
      },
      "articular": {
        "Articulación carpo-metacarpiana": [
          "Reducción",
          "Fijación con clavijas",
          "Reconstrucción ligamentaria"
        ],
        "Luxación metacarpofalángica": [
          "Reducción cerrada",
          "Fijación",
          "Reparación capsuloligamentaria"
        ]
      }
    },
    "tumoral": {
      "oseo": {
        "Tumor primario": [
          "Resección",
          "Injerto óseo",
          "Prótesis"
        ],
        "Tumor secundario": [
          "Resección",
          "Injerto óseo",
          "Prótesis"
        ]
      },
      "vascular": {
        "Tumor primario": [
          "Resección",
          "Ligadura"
        ],
        "Tumor secundario": [
          "Resección",
          "Ligadura"
        ]
      },
      "nervioso": {
        "Tumor primario": [
          "Resección",
          "Neurólisis",
          "Injerto"
        ],
        "Tumor secundario": [
          "Resección",
          "Neurólisis",
          "Injerto"
        ]
      },
      "musculo-tendinoso": {
        "Tumor primario": [
          "Resección",
          "Reparación tendinosa"
        ],
        "Tumor secundario": [
          "Resección",
          "Reparación tendinosa"
        ]
      },
      "articular": {
        "Tumor primario": [
          "Resección",
          "Artrodesis"
        ],
        "Tumor secundario": [
          "Resección",
          "Artrodesis"
        ]
      }
    },
    "degenerativo": {
      "articular": {
        "Artrosis carpo-metacarpiana (CMC)": [
          "Artrodesis",
          "Artroplastia",
          "Resección trapecio"
        ],
        "Inestabilidad metacarpofalángica": [
          "Reconstrucción ligamentaria",
          "Artrodesis",
          "Artroplastia"
        ]
      }
    },
    "infeccioso": {
      "todos": {
        "Osteomielitis metacarpiana": [
          "Debridamiento",
          "Resección",
          "Colgajo"
        ],
        "Artritis séptica CMC o MCP": [
          "Toilette articular",
          "Resección",
          "Colgajo"
        ]
      }
    }
  },
  "Falanges": {
    "traumatica": {
      "oseo": {
        "Fx de falange distal": [
          "Reducción y yeso",
          "Clavijas",
          "Tutores clásicos",
          "DET",
          "Placa-tornillo",
          "Alambre",
          "Otro"
        ],
        "Fx de falange media": [
          "Yeso",
          "Osteosíntesis con tornillos",
          "Clavijas",
          "Fijador externo",
          "Otro"
        ],
        "Fx de falange proximal": [
          "Yeso",
          "Osteosíntesis con placa",
          "Clavijas",
          "DET",
          "Otro"
        ],
        "Luxación interfalángica proximal o distal": [
          "Reducción cerrada",
          "Fijación transarticular",
          "Reparación ligamentaria",
          "Otro"
        ]
      },
      "vascular": {
        "Lesión arterial digital": [
          "Sutura",
          "Ligadura",
          "Interposición"
        ]
      },
      "nervioso": {
        "Nervios digitales": [
          "Neurorrafia",
          "Neurolisis",
          "Injerto"
        ]
      },
      "musculo-tendinoso": {
        "Lesión tendón extensor (tipo mallet)": [
          "Férula",
          "Sutura",
          "Tenodermodesis"
        ],
        "Lesión tendón flexor (tipo jersey)": [
          "Reparación primaria",
          "Injerto tendinoso",
          "Tenolisis"
        ]
      },
      "articular": {
        "Articulación interfalángica proximal (IFP)": [
          "Reducción",
          "Fijación",
          "Artrodesis"
        ],
        "Articulación interfalángica distal (IFD)": [
          "Reducción",
          "Fijación",
          "Artrodesis"
        ]
      }
    },
    "tumoral": {
      "oseo": {
        "Tumor primario": [
          "Resección",
          "Injerto",
          "Amputación parcial"
        ],
        "Tumor secundario": [
          "Resección",
          "Injerto",
          "Amputación parcial"
        ]
      },
      "vascular": {
        "Tumor vascular digital (glómico u otros)": [
          "Resección",
          "Ligadura"
        ],
        "Tumor secundario": [
          "Resección",
          "Ligadura"
        ]
      },
      "nervioso": {
        "Tumor de nervio digital": [
          "Resección",
          "Injerto"
        ],
        "Tumor secundario": [
          "Resección",
          "Injerto"
        ]
      },
      "musculo-tendinoso": {
        "Tumor tendinoso": [
          "Resección",
          "Reconstrucción tendinosa"
        ],
        "Tumor secundario": [
          "Resección",
          "Reconstrucción tendinosa"
        ]
      },
      "articular": {
        "Tumor articular interfalángico": [
          "Resección",
          "Artrodesis"
        ],
        "Tumor secundario": [
          "Resección",
          "Artrodesis"
        ]
      }
    },
    "degenerativo": {
      "articular": {
        "Artrosis interfalángica distal (IFD)": [
          "Artrodesis",
          "Artroplastia",
          "Toilette articular"
        ],
        "Artrosis interfalángica proximal (IFP)": [
          "Artrodesis",
          "Artroplastia",
          "Toilette articular"
        ],
        "Artrosis metacarpofalángica": [
          "Artroplastia",
          "Artrodesis",
          "Resección"
        ]
      }
    },
    "infeccioso": {
      "todos": {
        "Panadizo óseo (osteomielitis digital)": [
          "Debridamiento",
          "Resección falángica parcial",
          "Colgajo"
        ],
        "Artritis séptica interfalángica": [
          "Toilette articular",
          "Resección",
          "Colgajo"
        ]
      }
    }
  }
 
  
};